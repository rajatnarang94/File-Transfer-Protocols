...
import sys
import pickle
from constants import TYPE_ACK, TYPE_DATA, TYPE_EOF, ACK_HOST, ACK_PORT, RTT
...
def timeout_thread(timeout_th, frame):
    global last_ack_packet, SEND_HOST, SEND_PORT
    length_sliding_window = len(sliding_window)
    if last_ack_packet == last_send_packet-length_sliding_window:
        print("Timeout, sequence number = ", last_ack_packet + 1)
        thread_lock.acquire()
        for i in range(last_ack_packet + 1, last_ack_packet + 1 + length_sliding_window, 1):
            alarm(0)
            setitimer(ITIMER_REAL, RTT)
            packet = client_buffer.get(i)
            client_socket.sendto(packet, (SEND_HOST, SEND_PORT))
        thread_lock.release()
...
if __name__ == "__main__":
    if len(sys.argv) < 6:
        ...
        SEND_HOST, SEND_PORT, FILE_NAME, N, MSS = sys.argv[1], int(sys.argv[2]), sys.argv[3], int(sys.argv[4]), int(sys.argv[5])
